...
app.post('/api/generate', async (req, res) => {
  ...
  const upperWearText = upperWear && upperWear.length > 0 ? upperWear.join(', ') : '';
  const lowerWearText = lowerWear && lowerWear.length > 0 ? lowerWear.join(', ') : '';
  const shoesText = shoes && shoes.length > 0 ? shoes.join(', ') : '';
  const headAccessoriesText = headAccessories && headAccessories.length > 0 ? headAccessories.join(', ') : '';
  ...
  try {
    ...
    const detailedBreakdown = {
      upperWear: upperWearText || 'Stylish top piece',
      lowerWear: lowerWearText || 'Matching bottom',
      shoes: shoesText || 'Complementary footwear',
      accessories: accessoryText || 'Minimal accessories',
      headAccessories: headAccessoriesText || 'None',
      hairstyle: hairstyle || 'Natural style',
      colorPalette: `${colorTheme} theme with ${mainColor} as primary`,
      fabricDetails: fabric || 'Quality materials',
      seasonalNotes: `Perfect for ${season || 'any season'}`,
      occasionFit: `Ideal for ${occasion || 'various occasions'}`
    };
    const specs = {
      style,
      fabric,
